        ...
        <div className="space-y-12">
          {initiativesData.map((init) => {
            ...
            return (
              <div
                key={init.id}
                className="rounded-3xl bg-white/80 dark:bg-navy-950/50 border border-slate-200/80 dark:border-white/[0.08] p-6 sm:p-10 lg:p-12 backdrop-blur-2xl relative overflow-hidden shadow-sm dark:shadow-[inset_0_1px_1px_0_rgba(255,255,255,0.1),0_20px_50px_-10px_rgba(0,0,0,0.6)]"
              >
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-2xl bg-cyan-50 dark:bg-cyan/10 border border-cyan-200 dark:border-cyan/30 flex items-center justify-center text-cyan-700 dark:text-cyan backdrop-blur-md shadow-sm dark:shadow-[0_0_15px_rgba(0,229,255,0.2)]">
                    ...
                    <span className="text-xs font-mono text-cyan-700 dark:text-cyan bg-slate-100 dark:bg-white/[0.04] backdrop-blur-md px-3.5 py-1.5 rounded-2xl border border-slate-200 dark:border-white/[0.08] shadow-sm font-semibold">
                      {init.tagline}
                    </span>
                  </div>

                  <p className="text-slate-600 dark:text-slate-300 text-base leading-relaxed mb-8 max-w-4xl font-normal">
                    {init.description}
                  </p>

                  {/* Problem & Objective Glass Grid */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div className="p-6 rounded-3xl bg-red-50/60 dark:bg-white/[0.03] backdrop-blur-xl border border-red-200 dark:border-white/[0.07] shadow-sm">
                      <h4 className="text-xs font-mono uppercase tracking-widest text-red-600 dark:text-amber-400 font-bold mb-2 flex items-center gap-2">
                        <AlertCircle className="w-4 h-4 text-red-600 dark:text-amber-400" /> The Problem We Solve
                      </h4>
                    ...
                    <div className="p-6 rounded-3xl bg-cyan-50/60 dark:bg-white/[0.03] backdrop-blur-xl border border-cyan-200 dark:border-white/[0.07] shadow-sm">
                      <h4 className="text-xs font-mono uppercase tracking-widest text-cyan-700 dark:text-cyan font-bold mb-2 flex items-center gap-2">
                        <Target className="w-4 h-4 text-cyan-700 dark:text-cyan" /> Core Program Objective
                      </h4>
                      <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-300 leading-relaxed font-normal">
                        {init.objective}
                      </p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-6 border-t border-slate-200 dark:border-white/[0.06]">
                    <div>
                      <h4 className="text-xs font-mono uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-3 font-semibold">
                        How The Program Operates
                      </h4>
                      <ul className="space-y-2.5">
                        {init.howItWorks.map((step, i) => (
                          <li key={i} className="flex items-start gap-2 text-xs sm:text-sm text-slate-700 dark:text-slate-300 font-normal">
                            <span className="text-cyan-700 dark:text-cyan font-mono font-bold shrink-0">•</span>
                            <span>{step}</span>
                          </li>
                        ))}
                      </ul>
